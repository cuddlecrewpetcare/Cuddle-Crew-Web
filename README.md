...
Use the persistent Windows-first workflow in [`docs/local-development.md`](docs/local-development.md) and the repository/history safeguards in [`CONTRIBUTING.md`](CONTRIBUTING.md). For normal daily work, read `AGENTS.md`, run `npm run doctor` and `npm run check:git-safety`, and install nothing when the doctor passes. First-time or evidence-based repair uses `npm run setup:local`.
