...
export async function POST(request:Request){
 ...
 const address=clean((await request.json().catch(()=>({}))).address);if(address.length<6)return Response.json({error:'Enter a complete street address.'},{status:400});
 try{
  ...
  let travelContext:string|undefined;try{const route=await fetch('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',headers:{'Content-Type':'application/json','X-Goog-Api-Key':key!,'X-Goog-FieldMask':'routes.duration,routes.distanceMeters'},body:JSON.stringify({origin:{address:origin},destination:{address:`${normalized.city}, ${normalized.state} ${normalized.zip}`},travelMode:'DRIVE'})});if(route.ok){const data=await route.json() as {routes?:{duration?:string;distanceMeters?:number}[]},first=data.routes?.[0],seconds=Number((first?.duration||'').replace('s',''));if(first)travelContext=routeContext(seconds,first.distanceMeters||0)}}catch{}
