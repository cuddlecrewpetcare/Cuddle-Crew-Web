...
export function createContactPost(sendEmail:EmailSender=sendResendEmail,monotonicNow=()=>performance.now(),instantNow=()=>new Date()){
  return async function POST(request:Request){
    ...
    const turnstileStarted=monotonicNow();
    const verification=await verifyTurnstile(checked.turnstileToken||'',ip);
    if(!verification.ok){
      logProviderFailure('turnstile','verify',requestId,verification,monotonicNow()-turnstileStarted);
      return json({error:'Contact security is temporarily unavailable. Please email Lauren directly.'},{status:503});
    }
    logDiagnostic('DEBUG','provider.call_completed',{provider:'turnstile',operation:'verify',requestId,durationMs:monotonicNow()-turnstileStarted,result:verification.verified?'verified':'rejected'});
    ...
    const fingerprint=await contactFingerprint(data),now=monotonicNow();
    pruneAttempts(now);
    ...
    const attempt=prior||{
      ...
      ...(data.smsConsent?{smsConsentTimestamp:instantNow().toISOString()}:{}),
    };
    ...
    const businessStarted=monotonicNow();
    ...
    if(!businessResult.ok){
      logProviderFailure('resend','contact-notification',requestId,businessResult,monotonicNow()-businessStarted);
      return json({error:'Unable to send inquiry.'},{status:upstreamStatus(businessResult)});
    }
    logDiagnostic('INFO','provider.write_accepted',{provider:'resend',operation:'contact-notification',requestId,durationMs:monotonicNow()-businessStarted,result:'accepted',...(businessResult.providerRequestId?{providerRequestId:businessResult.providerRequestId}:{})});
    ...
    const confirmationStarted=monotonicNow();
    ...
    if(!confirmationResult.ok)logProviderFailure('resend','visitor-confirmation',requestId,confirmationResult,monotonicNow()-confirmationStarted);
    else logDiagnostic('DEBUG','provider.write_accepted',{provider:'resend',operation:'visitor-confirmation',requestId,durationMs:monotonicNow()-confirmationStarted,result:'accepted',...(confirmationResult.providerRequestId?{providerRequestId:confirmationResult.providerRequestId}:{})});
    return json({ok:true});
  };
}
