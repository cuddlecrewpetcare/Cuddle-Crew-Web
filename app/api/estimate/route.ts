...
export async function POST(request:Request){
 const limit=rateLimit(`estimate:${clientKey(request)}`,60,5*60_000);if(!limit.allowed)return Response.json({error:'Please wait and try again.'},{status:429,headers:{'Retry-After':String(limit.retryAfter)}});
 const parsed=await readJsonObject(request,8_000,['pets','service','start','end','blocks','midday','zip','travelTier']);if(!parsed.ok)return Response.json({error:parsed.error},{status:parsed.status});
 const value=parsed.value,pets=Array.isArray(value.pets)?value.pets:[];
 if(pets.length<1||pets.length>8||!pets.every(p=>p&&typeof p==='object'&&petTypes.has((p as {type?:PetType}).type as PetType)))return Response.json({error:'Choose a valid pet household.'},{status:400});
 ...
 if(!services.has(service)||!middayServices.has(midday)||(travelTier!==undefined&&!travelTiers.has(travelTier)))return Response.json({error:'Choose valid service options.'},{status:400});
 if(!validDate(value.start)||!validDate(value.end)||value.end<value.start||(Date.parse(`${value.end}T12:00:00Z`)-Date.parse(`${value.start}T12:00:00Z`))/86_400_000>366||typeof value.zip!=='string'||!/^\d{5}$/.test(value.zip))return Response.json({error:'Choose valid dates and ZIP.'},{status:400});
 ...
 const output=calculateEstimate(input);if(!output.result)return Response.json(output,{headers:{'Cache-Control':'no-store'}});
 const{reviewReasons:privateReasons,...publicResult}=output.result;void privateReasons;
 return Response.json({issues:output.issues,result:publicResult satisfies PublicEstimateResult},{headers:{'Cache-Control':'no-store'}});
}
