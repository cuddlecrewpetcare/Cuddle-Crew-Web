...
export async function POST(request:Request){
 const requestId=createRequestId(),json=(body:unknown,init:ResponseInit={})=>jsonWithRequestId(body,requestId,init);
 ...
 const parsed=await readJsonObject(request,8_000,['pets','service','start','end','blocks','midday','zip','travelTier']);if(!parsed.ok)return json({error:parsed.error},{status:parsed.status});
 const value=parsed.value,pets=Array.isArray(value.pets)?value.pets:[];
 if(pets.length<1||pets.length>8||!pets.every(p=>p&&typeof p==='object'&&petTypes.has((p as {type?:PetType}).type as PetType)))return json({error:'Choose a valid pet household.'},{status:400});
 ...
 if(!validDate(value.start)||!validDate(value.end)||value.end<value.start||(Date.parse(`${value.end}T12:00:00Z`)-Date.parse(`${value.start}T12:00:00Z`))/86_400_000>366||typeof value.zip!=='string'||!/^\d{5}$/.test(value.zip))return json({error:'Choose valid dates and ZIP.'},{status:400});
 ...
 const output=calculateEstimate(input);if(!output.result)return json(output,{headers:{'Cache-Control':'no-store'}});
 const{reviewReasons:privateReasons,...publicResult}=output.result;void privateReasons;
 return json({issues:output.issues,result:publicResult satisfies PublicEstimateResult},{headers:{'Cache-Control':'no-store'}});
}
