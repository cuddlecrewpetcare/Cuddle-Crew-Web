...
export default function ContactTools({turnstileSiteKey=''}:{turnstileSiteKey?:string}){
  ...
  return <div className="contact-tools">
    ...
    <form className="inquiry-builder" onSubmit={submit}><p className="eyebrow">Initial inquiry</p><h2>Ask Lauren a question.</h2><p>Share the basics below. This does not register you, create a reservation, or guarantee availability.</p><label>Your name <span aria-hidden="true">(required)</span><input required maxLength={80} autoComplete="name" value={name} onChange={e=>setName(e.target.value)}/></label><label>Your email <span aria-hidden="true">(required)</span><input required maxLength={254} autoComplete="email" type="email" value={replyTo} onChange={e=>setReplyTo(e.target.value)}/></label><div className="inquiry-pair"><label>Phone <small>Optional</small><input maxLength={30} autoComplete="tel" type="tel" value={phone} onChange={e=>setPhone(e.target.value)}/></label><label>Service ZIP <small>Optional</small><input maxLength={10} inputMode="numeric" autoComplete="postal-code" value={zip} onChange={e=>setZip(e.target.value)}/></label></div><label>What is your question about?<select value={topic} onChange={e=>setTopic(e.target.value)}><option>Availability or scheduling</option><option>Services and care options</option><option>Pricing or an estimate</option><option>Service area or travel fees</option><option>Meet-and-greet or registration</option><option>Medication or special care</option><option>Existing client question</option><option>Other</option></select></label><label>What would you like to ask? <span aria-hidden="true">(required)</span><textarea required minLength={10} maxLength={3000} rows={7} value={message} onChange={e=>setMessage(e.target.value)} placeholder="Pet type, dates, care needs, and any questions…"/></label><label className="form-trap" aria-hidden="true">Website<input name="website" tabIndex={-1} autoComplete="off"/></label><p className="form-privacy">Please don’t include door or alarm codes, payment details, veterinary records, or detailed medical information. Use the secure client portal for those details.</p>{siteKey&&<TurnstileWidget siteKey={siteKey} onToken={receiveToken} resetKey={turnstileReset}/>}<button className="button" type="submit" disabled={status==='sending'||Boolean(siteKey&&!turnstileToken)}>{status==='sending'?'Sending…':'Send inquiry'}</button>{status==='sent'&&<p className="form-status success" role="status">Thanks—your inquiry was sent to Lauren. Routine questions are usually answered within one business day. If you need care within 48 hours, submit your request through the client portal and call 916-252-3550. Text messaging is not currently available.</p>}{status==='error'&&<div ref={errorRef} className="form-status error" role="alert" tabIndex={-1}><p>{errorMessage}</p><button type="button" className="text-link" onClick={mailDraft}>Open it as an email draft instead →</button></div>}</form>
  </div>;
}
