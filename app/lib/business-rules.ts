...
/** Approved holiday/peak periods that overlap the requested calendar year. */
export const publicHolidays=(year:number)=>business.holidayPeriods.filter(period=>period.start.slice(0,4)<=String(year)&&period.end.slice(0,4)>=String(year));
export const holidayForDate=(date:string)=>/^\d{4}-\d{2}-\d{2}$/.test(date)?business.holidayPeriods.find(period=>period.start<=date&&period.end>=date):undefined;
