...
export const contactKeys=['name','replyTo','phone','smsConsent','zip','topic','message','website','startedAt','turnstileToken'] as const;
const emailPattern=/^[^\s@\r\n]+@[^\s@\r\n]+\.[^\s@\r\n]+$/;
const text=(value:unknown)=>typeof value==='string'?value.trim():'';
export const escapeHtml=(value:string)=>value.replace(/[&<>"']/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[char]!));

export function validateContact(value:Record<string,unknown>){
  const name=text(value.name),replyTo=text(value.replyTo).toLowerCase(),rawPhone=text(value.phone),smsConsent=value.smsConsent===true,zip=text(value.zip),topic=text(value.topic),message=text(value.message),website=text(value.website),turnstileToken=text(value.turnstileToken),startedAt=Number(value.startedAt);
  ...
  if(name.length<2||name.length>80||/[\r\n]/.test(name)||replyTo.length>254||!emailPattern.test(replyTo)||rawPhone&&!phone||smsConsent&&!phone||zip&&!/^\d{5}$/.test(zip)||!contactTopics.includes(topic as typeof contactTopics[number])||message.length<10||message.length>3000||!Number.isFinite(startedAt))return{ok:false as const,error:'Please check the required fields.'};
