...
export type EstimateReviewReason='travel'|'complex-care'|'household'|'unusual-species'|'small-animal-overnight'|'extended-overnight'|'long-stay'|'short-notice'|'continuous-care';
export type EstimateResult={total:number|null;serviceSubtotal:number;base:number;petFee:number;units:number;holidayFee:number;holidayCount:number;potentialShortFee:number;shortCount:number;sameDayCount:number;travelFee:number|null;travelTier?:TravelTierKey;addOn:number;addOnUnits:number;reviewRequired:boolean;reviewReasons:EstimateReviewReason[]};
