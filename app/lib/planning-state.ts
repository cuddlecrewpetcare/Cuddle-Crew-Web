...
const validIso=(value:unknown):value is string=>{if(typeof value!=='string'||!/^\d{4}-\d{2}-\d{2}$/.test(value))return false;const parsed=new Date(`${value}T12:00:00Z`);return!Number.isNaN(parsed.getTime())&&parsed.toISOString().slice(0,10)===value};
export const sanitizePlanningState=(input:Record<string,unknown>):PlanningState=>({
 ...
 start:validIso(input.start)?input.start:undefined,end:validIso(input.end)?input.end:undefined,
