...
>
> Website use: Cancellation and refund FAQ, booking-change behavior, holiday cancellation rules, early return, trip extensions, reservation-payment treatment, and cancellation-related UX.

## Purpose

This document contains the current approved Cancellation, Booking Change & Refund Policy used by Cuddle Crew Pet Care LLC through Precise Petcare.

This document is the authoritative client-facing source for cancellation, booking-change, reservation-payment, refund, credit, early-return, trip-extension, and related rules covered by this Policy.

Website copy, booking functionality, cancellation explanations, FAQs, and client-facing workflows must remain consistent with this Policy.
...
Routine booking communications, cancellation requests, booking-change requests, care instructions, confirmations, and operational updates may be provided through Precise Petcare, email, telephone, or another communication method designated by the Contractor.

A cancellation or booking-change request will be considered received when the Contractor receives the Client’s written request.
...
A compassionate exception, refund, credit, reduced cancellation charge, reservation-payment accommodation, or other courtesy provided by the Contractor in a particular circumstance will not constitute a waiver or modification of this Policy.
...
- Daytime Services cancelled twenty-four (24) hours or more before the scheduled Service do not incur a cancellation fee, and amounts already paid for the cancelled Service are refundable or creditable.
...
- A reservation payment is applied toward the booking balance and is not automatically non-refundable.
- Cancellation rules determine what amount, if any, may be retained after cancellation.
- Payment deadlines and cancellation deadlines are separate concepts and must not be presented as though they are interchangeable.
- Failure to make a payment by a particular payment deadline does not automatically eliminate a cancellation charge otherwise due under this Policy.
- A request to remove, shorten, move, or otherwise modify a confirmed Service may be treated as a cancellation of the affected capacity unless another arrangement is approved.
- Schedule changes are subject to availability.
- The website must not promise that a confirmed Service can always be moved to another date or time.
- If a Client returns home early, remaining Services no longer required are treated as cancelled when Cuddle Crew receives notice.
- Trip-extension requests are subject to availability and applicable pricing, including any qualifying short-notice, same-day, holiday, or travel modifier.
- A delayed departure does not automatically cancel or move already reserved Services.
- A pet being temporarily absent from the home does not automatically eliminate the reservation or applicable cancellation terms.
- Genuine emergencies and unforeseen circumstances may qualify for discretionary compassionate accommodations.
- Compassionate exceptions are discretionary and must not be represented as guaranteed entitlements.
- A previous exception, waiver, refund, credit, or reduced charge does not establish a permanent exception to the Policy.
- Repeated late cancellations or booking changes may result in future booking conditions or declined future reservations.
- If cancelled capacity is successfully rebooked, Cuddle Crew may provide an additional courtesy adjustment, but the website must not promise that cancelled capacity will be rebooked or that an adjustment will be issued.
- If Cuddle Crew cancels and neither Cuddle Crew nor an approved substitute performs the Service, the Client receives a full refund or credit for the unperformed Service.
- Website refund timing must not promise a specific bank-processing timeline unless supported by the applicable payment processor.
- Precise Petcare bookings, quotes, invoices, and approved records may establish booking-specific reservation-payment requirements, payment deadlines, pricing, and operational details.
- Booking-specific records supplement this Policy but do not silently alter its legal cancellation terms.
- Website cancellation calculators, FAQs, booking-change tools, or account interfaces must derive their behavior from this CURRENT / APPROVED Policy rather than duplicating conflicting cancellation rules elsewhere in the codebase.
- If a website feature cannot confidently determine which cancellation category applies, it should refer the request for review rather than automatically applying an unsupported fee or refund.
- Public-facing summaries may simplify the wording for readability but must preserve the actual deadlines, percentages, exceptions, and distinctions in this Policy.
- If existing website behavior conflicts with this Policy, the conflict must be flagged and corrected rather than weakening or rewriting the Policy to match the existing implementation.

## Codex Guardrails

- Do not infer missing cancellation, refund, payment, or booking-change rules.
- Do not treat TODOs, examples, old website copy, or superseded policy language as current business rules.
- Treat the Authoritative Content section of this CURRENT / APPROVED document as the source of truth for the subjects it covers.
- Do not alter, weaken, broaden, narrow, reinterpret, or replace an authoritative cancellation rule merely to make implementation easier.
- Do not change a deadline or percentage for UX convenience.
- Do not convert "may be charged" into "will always be charged" unless the authoritative language expressly requires that result.
- Do not convert a discretionary compassionate exception into a guaranteed client entitlement.
- Do not automatically waive a fee merely because an exception was granted previously.
- Do not assume reservation payments are non-refundable.
- Do not confuse the reservation-payment amount with the amount ultimately retained after cancellation.
- Do not confuse payment deadlines with cancellation deadlines.
- Do not infer a reservation-payment percentage if the current approved booking, quote, invoice, or Pricing, Fees & Surcharge Policy does not specify one.
- Do not invent holiday or peak-date periods.
- Use the CURRENT / APPROVED Holiday / Peak-Date Calendar when a website feature requires exact qualifying dates.
- Do not promise that a booking-change request can always be accommodated.
- Do not promise that cancelled capacity will be rebooked.
- Do not promise a compassionate exception.
- Do not expose private scheduling or capacity information when explaining why a cancellation policy applies.
- Do not make existing website code authoritative over this Policy.
- If another CURRENT / APPROVED source materially conflicts with this document, flag the conflict for human review rather than silently reconciling it.
- If a booking-specific rule expressly accepted before confirmation differs from a default rule where this Policy permits booking-specific terms, preserve that distinction.
- Do not expose actual client travel dates, cancellation history, payment status, or booking records publicly.
- When website logic cannot determine the appropriate outcome from authoritative information, require human review rather than inventing an outcome.
