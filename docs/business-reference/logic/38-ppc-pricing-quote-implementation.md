...
It does **not** replace the CURRENT / APPROVED Pricing, Fees & Surcharge Policy. If a dollar amount in this implementation reference conflicts with `core/03-pricing-fees-surcharge-policy.md`, the client-facing Pricing Policy controls.
...
If the care requires more than approximately 15 supplemental minutes, use the appropriate full 30-, 60-, or 90-minute service structure.
...
## 8. Travel / Service-Area Implementation
...
## 9. Holiday / Peak-Date Implementation
...
## 10. Reservation Payment Working Rule
...
## 11. View-Only Quote Workflow

A separate quote calculator/document may be used before a Client's booking is finalized.

Preferred workflow:

1. calculate the proposed service structure using the approved pricing rules;
2. generate a polished **view-only** quote or PDF with final Client-facing line-item totals;
3. do not collect payment through the external quote document;
4. do not expose PPC internal base/modifier mechanics unless needed;
5. the quote may show the 25% reservation-payment amount and remaining balance informationally;
6. after Client acceptance, create/confirm the actual schedule and invoice in PPC;
7. collect payment through PPC/Stripe under the normal workflow;
8. allow PPC invoice/payment data to sync to QuickBooks for accounting;
9. if useful, upload the accepted/final quote PDF to the Client's PPC file for documentation.

Only the accepted/final quote should normally be retained in the Client file; calculation drafts do not need to be stored there.

## 12. Quote Is Not a Booking
...
- **QuickBooks:** accounting/bookkeeping mirror and financial reporting, not the operational scheduling source of truth;
- **External quote builder:** optional view-only pre-booking presentation/calculation tool.

Do not intentionally create a second payment workflow in an external quote tool when the goal is only to present a quote.

Do not assume an edit made only in QuickBooks will become an operational PPC booking change.

## 14. Client-Facing Presentation

Client-facing quotes should show understandable final service lines, for example:

- 3 Overnight Stays — Dog + 2 Cats;
- 3 Dedicated 30-Minute Dog Walks;
- 3 Cat Care Visits;
- Extended Service Area — 3 arrivals;
- Holiday / Peak-Date Fee where applicable;
- Booking Total;
- Reservation Payment Due to Hold Dates;
- Remaining Balance.
...
- Exact 2026 holiday dates come from `logic/36-holiday-peak-date-calendar.md`.
