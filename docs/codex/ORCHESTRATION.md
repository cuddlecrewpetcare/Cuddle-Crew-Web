...
Before meaningful delegation assess TASK_COMPLEXITY (LOW/MEDIUM/HIGH/EXCEPTIONAL), AMBIGUITY, RISK (LOW/MEDIUM/HIGH/CRITICAL), DETERMINISM, CROSS_FILE_REASONING, CROSS_SYSTEM_REASONING, CONTEXT_DEMAND, REVERSIBILITY, TIME_SENSITIVITY and COST_SENSITIVITY (other scales LOW/MEDIUM/HIGH). Then record MODEL, REASONING, SPEED and WHY in the assignment. A concise justification suffices for trivial work.
...
On cancellation issue TASK_CANCELLED and stop/interrupt the worker through supported controls; confirm it stopped before reassigning shared scope. If control is blocked, mark cancellation pending, preserve evidence and do not start competing writes. Before closing/archiving, preserve commits and evidence; archive/delete must not discard unrecovered work. A replacement Orchestrator reconstructs from Git, status, RUN/phase records, branches and native task state, replaces the runtime routing record, reconciles unresolved ownership and preserves existing authority. It never depends on one chat's memory.
