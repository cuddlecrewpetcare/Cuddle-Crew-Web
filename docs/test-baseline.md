...
F4 added 3 read-only supply-chain guard tests, bringing that checkpoint to 99 Node tests without changing the 96-test application baseline. F5 added 3 high-value API/security regressions, bringing the baseline to 102. F6 adds 8 focused integration regressions for the Resend write gate/request contract/timeout normalization, Turnstile response validation, malformed calendar fallback, contact unknown-outcome idempotency and partial failure, and Maps key/response safety. The current baseline is therefore 110 Node tests and 10 Playwright tests. Standard validation runs both `npm run check:supply-chain` and `npm run check:integrations`; the supply-chain check retains its documented warning for five optional/WASM entries that a clean Windows `npm ci` causes `npm ls` to label extraneous.
