...
test('all public routes, metadata routes, legacy redirects, and 404 return expected status',async({request})=>{
  ...
  for(const [route,target] of [['/about','/#meet-lauren']] as const){const response=await request.get(route,{maxRedirects:0});expect(response.status(),route).toBe(308);expect(response.headers().location).toBe(`https://www.cuddlecrewpetcare.com${target}`)}
...
test('launch pages retain landmarks, headings, links, and progressive public content',async({page})=>{
  ...
  await expect(page.getByRole('heading',{name:'Clear prices for standard care.'})).toBeVisible();
...
test('dedicated planning tools resolve at their canonical anchors',async({page})=>{
  await page.goto('/rates#estimate');
  await expect(page.getByRole('heading',{name:'Build a preliminary planning estimate.'})).toBeInViewport();
  await expect(page.locator('.estimate-fields')).toBeVisible();
