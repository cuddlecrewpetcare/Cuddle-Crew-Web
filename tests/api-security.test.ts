...
test('contact validation normalizes safe values and rejects injection or bad fields',async()=>{
 const base={name:'Lauren Test',replyTo:'USER@Example.com',phone:'(916) 555-1212',zip:'95814',topic:'Availability or scheduling',message:'Please tell me about availability.',website:'',startedAt:Date.now()-4000,turnstileToken:''};
 ...
 if(valid.ok&&!valid.honeypot){const fingerprintData={name:valid.data.name,replyTo:valid.data.replyTo,topic:valid.data.topic,message:valid.data.message};assert.equal(await contactFingerprint(fingerprintData),await contactFingerprint(fingerprintData))}
});

test('contact endpoint accepts valid requests, suppresses duplicates, and exposes no unsupported method',async()=>{
 resetRateLimitsForTests();const previousKey=process.env.RESEND_API_KEY,previousSite=process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY,previousSecret=process.env.TURNSTILE_SECRET_KEY,originalFetch=globalThis.fetch;process.env.RESEND_API_KEY='test-key';delete process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY;delete process.env.TURNSTILE_SECRET_KEY;let calls=0;globalThis.fetch=(async()=>{calls++;return new Response('{}',{status:200})}) as typeof fetch;
 const body={name:'Route Tester',replyTo:'route@example.com',phone:'',zip:'95814',topic:'Other',message:'A valid route-level test message.',website:'',startedAt:Date.now()-4000,turnstileToken:''};
 try{const request=()=>new Request('https://example.test/api/contact',{method:'POST',headers:{'content-type':'application/json','cf-connecting-ip':'203.0.113.20'},body:JSON.stringify(body)});const first=await contactRoute.POST(request()),second=await contactRoute.POST(request());assert.equal(first.status,200);assert.deepEqual(await second.json(),{ok:true,duplicate:true});assert.equal(calls,2);assert.equal('GET' in contactRoute,false)}finally{globalThis.fetch=originalFetch;if(previousKey===undefined)delete process.env.RESEND_API_KEY;else process.env.RESEND_API_KEY=previousKey;if(previousSite===undefined)delete process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY;else process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY=previousSite;if(previousSecret===undefined)delete process.env.TURNSTILE_SECRET_KEY;else process.env.TURNSTILE_SECRET_KEY=previousSecret}
});
