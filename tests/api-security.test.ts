...
test('contact endpoint preserves non-consent and rejects forged consent metadata',async()=>{
 resetRateLimitsForTests();const previousKey=process.env.RESEND_API_KEY,previousSite=process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY,previousSecret=process.env.TURNSTILE_SECRET_KEY,originalFetch=globalThis.fetch;process.env.RESEND_API_KEY='test-key';delete process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY;delete process.env.TURNSTILE_SECRET_KEY;const sentBodies:string[]=[];globalThis.fetch=(async(_input,init)=>{sentBodies.push(String(init?.body||''));return new Response('{}',{status:200})}) as typeof fetch;
 ...
 try{const unchecked=await contactRoute.POST(new Request('https://example.test/api/contact',{method:'POST',headers:{'content-type':'application/json','cf-connecting-ip':'203.0.113.21'},body:JSON.stringify(base)}));assert.equal(unchecked.status,200);assert.equal(sentBodies.length,2);assert.match(sentBodies[0],/SMS consent: Not granted/);assert.doesNotMatch(sentBodies[0],/SMS consent source|SMS consent timestamp/);const forged=await contactRoute.POST(new Request('https://example.test/api/contact',{method:'POST',headers:{'content-type':'application/json','cf-connecting-ip':'203.0.113.22'},body:JSON.stringify({...base,name:'Forged Metadata',replyTo:'forged@example.com',smsConsentTimestamp:'2099-01-01T00:00:00.000Z',smsConsentSource:'verbal_consent'})}));assert.equal(forged.status,400);assert.equal(sentBodies.length,2)}finally{globalThis.fetch=originalFetch;if(previousKey===undefined)delete process.env.RESEND_API_KEY;else process.env.RESEND_API_KEY=previousKey;if(previousSite===undefined)delete process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY;else process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY=previousSite;if(previousSecret===undefined)delete process.env.TURNSTILE_SECRET_KEY;else process.env.TURNSTILE_SECRET_KEY=previousSecret}
});
...
test('contact provider failure is bounded to one attempted delivery and returns a safe error',async()=>{
 resetRateLimitsForTests();const previousKey=process.env.RESEND_API_KEY,previousSite=process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY,previousSecret=process.env.TURNSTILE_SECRET_KEY,originalFetch=globalThis.fetch,originalError=console.error;process.env.RESEND_API_KEY='test-key';delete process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY;delete process.env.TURNSTILE_SECRET_KEY;let calls=0;globalThis.fetch=(async()=>{calls++;return new Response('{}',{status:503})}) as typeof fetch;console.error=()=>{};
